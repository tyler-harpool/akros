...
(async () => {
  ...
  try {
    ...
    // Extract and display only the recommended bets section for quick reference
    if (completeAnalysis.includes('RECOMMENDED BETS')) {
      console.log('\n🎯 QUICK VIEW - RECOMMENDED BETS:\n');
      ...
      console.log(betsOnly);

      // Save bets to a file for easy reference
      const fs = require('fs');
      try {
        fs.writeFileSync('recommended_bets.txt', 'RECOMMENDED BETS\n' + betsOnly);
        console.log('\nRecommended bets saved to recommended_bets.txt');
      } catch (err) {
        console.error('Could not save bets to file:', err);
      }
    }
  } catch (error) {
